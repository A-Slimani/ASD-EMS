...
const Content = () => {
  ...
  const handleSubmit = e => {
    ...
    e.preventDefault();
    var s = window.confirm("Do you want add new roster with the entered information?\n\nSelect OK to proceed\n\nSelect CANCEL to reset form");
    if (s === true) {
      ...
      else if (rosterdate === "") { alert("Employement Date must be select"); }
      else if ((new Date().getDate() > new Date(rosterdate).getDate())) { alert("Invalid date"); }
      else if ((new Date().getMonth() > new Date(rosterdate).getMonth())) { alert("Invalid month"); }
      else if ((new Date().getFullYear() > new Date(rosterdate).getFullYear())) { alert("Invalid year"); }
      else { onSubmit(); }
    }
