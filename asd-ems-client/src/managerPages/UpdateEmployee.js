...
const Content = () => {
  ...
  useEffect(() => {
    employeeService.get(match.params.id).then(employees => setEmployee(employees));
  }, []);
  ...
  // updates the employee and redirects them back to the userlist page
  const onSubmit = async e => {
    console.log(match.params.id);
    console.log('employee update: ', employee);
    employeeService.update(match.params.id, employee);
  ...
  function handleSubmit(e) {
    ...
    var s = window.confirm(
      'Do you update the information with entered data?\n\nSelect OK to proceed\n\nSelect CANCEL to remain on the page'
    );
    if (s === true) {
      ...
      <div>
        <form id="updateform" name="updateform" onSubmit={e => onSubmit(e)}>
          <h1 style={{ textAlign: 'center', fontSize: 30, fontWeight: 'bold' }}>
            {' '}
            Update {showEmployee().fname + ' ' + showEmployee().lname}
          </h1>
          ...
          <select
            ...
            className="formtextfield"
            value={employtype}
            defaultValue={showEmployee().employtype}
            onChange={e => onInputChange(e)}>
          ...
          <select
            ...
            className="formtextfield"
            value={dept}
            defaultValue={showEmployee().dept}
            onChange={e => onInputChange(e)}
            required>
            <option value="select"> -- Select one -- </option>
          ...
          <div style={{ textAlign: 'center', paddingTop: 10 }}>
            <button type="submit" id="submit" className="button" onClick={handleSubmit}>
              {' '}
              Update <b>{showEmployee().username}</b>{' '}
            </button>{' '}
          </div>
        </form>
      </div>
